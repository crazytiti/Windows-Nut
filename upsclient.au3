...
Func prefGui()
	...
	$chReConnect = GUICtrlCreateCheckbox("$ReConnect", 334, 256, 17, 17,BitOr($BS_AUTOCHECKBOX,$WS_TABSTOP ),$WS_EX_STATICEDGE )
	GUICtrlCreateLabel("Re-establish connection", 217, 256, 115, 17)
	if GetOption("reconnect") == 0 Then
		GuiCtrlSetState($chReConnect , $GUI_UNCHECKED)
	Else
		GuiCtrlSetState($chReConnect , $GUI_CHECKED)
	EndIf
...
;	GUICtrlCreateTabItem("")
	...
	EndIf
    GUICtrlCreateLabel("Minimize on startup", 16, 64, 99, 17, BitOR($SS_BLACKRECT,$SS_GRAYFRAME,$SS_LEFTNOWORDWRAP))
	$chMinimizeOnStartup = GUICtrlCreateCheckbox("$chMinimizeOnStartup", 224, 60, 17, 17,BitOr($BS_AUTOCHECKBOX,$WS_TABSTOP ),$WS_EX_STATICEDGE)
	if GetOption("minimizeonstartup") == 0 Then
		GuiCtrlSetState($chMinimizeOnStartup , $GUI_UNCHECKED)
	Else
		GuiCtrlSetState($chMinimizeOnStartup , $GUI_CHECKED)
	EndIf
	...
	While 1
		...
		Switch $nMsg1
			...
			Case $Bapply
				...
				EndIf
			    $minimizeonstartup = GuiCtrlRead($chMinimizeOnStartup)
				if $minimizeonstartup == $GUI_CHECKED Then
					SetOption("minimizeonstartup",1 , "number")
				Else
					SetOption("minimizeonstartup",0 , "number")
				EndIf
				$reconnect = GuiCtrlRead($chReConnect)
				if $reconnect == $GUI_CHECKED Then
					SetOption("reconnect",1 , "number")
				Else
					SetOption("reconnect",0 , "number")
				EndIf
...
Func OpenMainWindow($windowsstate)
...
func mainLoop()
	...
	$minimizeonstartup = GetOption("minimizeonstartup")
	$reconnect = GetOption("reconnect")
	$lastreconnettime = _Timer_Init()
	...
	EndIf
	;refresh tray icon tolltip
	$tooltiptray = "Status " & $upsstatus & chr(10)
	$tooltiptray = $tooltiptray & "Batt. " & $battCh & "%" & chr(10)
	$tooltiptray = $tooltiptray & "Load " & $upsLoad & "%" & chr(10)
	TraySetToolTip($tooltiptray)
	;try to reconnect every 10 seconds if checked
	if $socket ==0 and $reconnect ==1 and 10000 < _Timer_Diff($lastreconnettime) Then
		AdlibUnRegister()
		ConnectServer() ;;aaaaa
		Opt("TCPTimeout",3000)
		GetUPSInfo()
...
$minimizetray = GetOption("minimizetray")
$minimizeonstartup = GetOption("minimizeonstartup")
if ($minimizeonstartup ==1) Then
	if ($minimizetray ==1) Then
		OpenMainWindow(@SW_HIDE)
	Else
		OpenMainWindow(@SW_MINIMIZE)
	EndIf
Else
	OpenMainWindow(@SW_SHOW)
EndIf
